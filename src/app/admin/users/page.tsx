        ...
        >
          ...
          <div className="mb-5">
            ...
            <input
              ...
              onFocus={(e) => { if (!mismatch) e.target.style.borderColor = 'var(--brand)'; }}
...
export default function AdminUsersPage() {
  ...
  const getPlanBadge = (plan?: string, role?: string) => {
    ...
    if (isPro) return (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold"
        style={{ background: 'linear-gradient(135deg,rgba(249,115,22,0.15),rgba(234,88,12,0.1))', color: 'var(--brand)', border: '1px solid rgba(249,115,22,0.3)' }}>
                              ...
                              >
                                {(['reader', 'editor', 'admin'] as UserRole[]).map((role) => (
                                  <button
                                    ...
                                    style={{ color: user.role === role ? 'var(--brand)' : 'var(--text-primary)' }}
                                  >
                                    {role === 'admin' && '🛡️'}
                                    {role === 'editor' && '✍️'}
                                    {role === 'reader' && '👤'}
