        ...
        <div className="flex-1 min-w-0 max-w-[760px]">
          ...
          <div>
            <motion.article initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="px-4 py-4 border-b" style={{ borderColor: 'var(--border-primary)' }}>
              ...
              {showShareConfirm && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="fixed bottom-20 left-1/2 -translate-x-1/2 px-4 py-2 rounded-full text-sm font-medium z-50" style={{ background: 'var(--brand)', color: '#fff' }}>
            ...
            <div id="comments" className="scroll-mt-20">
              {isAuthenticated && currentUser ? (
                <div className="flex gap-3 px-4 py-4 border-b" style={{ borderColor: 'var(--border-primary)' }}>
                  ...
                  <div className="flex-1">
                    <textarea value={newComment} onChange={(e) => setNewComment(e.target.value)} placeholder="Viết bình luận..." rows={2}
                      ...
                      onFocus={(e) => (e.target.style.borderColor = 'var(--brand)')} onBlur={(e) => (e.target.style.borderColor = 'var(--border-primary)')}
                    />
                    <div className="flex justify-end mt-2">
                      <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={handleComment} disabled={!newComment.trim()}
                        className="px-4 py-1.5 rounded-full text-sm font-semibold text-white disabled:opacity-40" style={{ background: 'var(--brand)' }}>Gửi</motion.button>
                    </div>
                  </div>
                </div>
              ) : (
                <div className="px-4 py-6 border-b text-center" style={{ borderColor: 'var(--border-primary)' }}>
                  ...
                  <button onClick={() => router.push('/login')} className="px-6 py-2 rounded-full font-semibold text-white text-sm" style={{ background: 'var(--brand)' }}>Đăng nhập ngay</button>
                </div>
              )}
            </div>
                        ...
                        <div className="mt-3 flex gap-2">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex justify-end gap-2 mt-1">
                              ...
                              <button onClick={() => handleReply(comment.id)} disabled={!replyText.trim()} className="text-xs px-3 py-1 rounded-full text-white disabled:opacity-40" style={{ background: 'var(--brand)' }}>Trả lời</button>
                            </div>
                          </div>
                        </div>
                      )}
