...
import { ArrowLeft, Heart, MessageCircle, Share2, Clock, Check } from 'lucide-react';
...
export default function ArticleDetailPage() {
  ...
  const { currentUser, isAuthenticated, isLiked, toggleLike, isShared, toggleShare } = useAuth();
  const articleId = params.id as string;
  const article = mockArticles.find((a) => a.id === articleId);
  const [comments, setComments] = useState<Comment[]>(mockComments.filter((c) => c.articleId === articleId));
  ...
  const handleComment = () => {
    ...
    const comment: Comment = { id: generateId(), content: newComment, author: currentUser, articleId: article.id, parentId: null, createdAt: new Date().toISOString(), likesCount: 0, replies: [] };
  ...
  const handleReply = (parentId: string) => {
    ...
    const reply: Comment = { id: generateId(), content: replyText, author: currentUser, articleId: article.id, parentId, createdAt: new Date().toISOString(), likesCount: 0 };
  ...
  return (
    <div className="min-h-screen lg:pl-[275px]" style={{ background: 'var(--bg-primary)' }}>
      ...
      <main className="flex-1 min-h-screen max-w-[760px]">
        ...
        <div>
        <motion.article initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="px-4 py-4 border-b">
          ...
          <div className="flex items-center gap-2 mt-4 pt-4 border-t">
            ...
            <span className="text-sm px-2 py-0.5 rounded-full ml-2" style={{ background: `${article.topic.color}20`, color: article.topic.color }}>
              {article.topic.icon} {article.topic.name}
            </span>
          </div>

          <div className="flex items-center gap-6 mt-3 pt-3 border-t">
            <span className="text-sm" style={{ color: 'var(--text-secondary)' }}><strong style={{ color: 'var(--text-primary)' }}>{formatNumber(article.commentsCount)}</strong> bình luận</span>
            <span className="text-sm" style={{ color: 'var(--text-secondary)' }}><strong style={{ color: 'var(--text-primary)' }}>{formatNumber(article.likesCount + (liked ? 1 : 0))}</strong> lượt thích</span>
            <span className="text-sm" style={{ color: 'var(--text-secondary)' }}><strong style={{ color: 'var(--text-primary)' }}>{formatNumber(article.sharesCount)}</strong> chia sẻ</span>
          </div>
        ...
        <div id="comments" className="scroll-mt-20">
          {isAuthenticated && currentUser ? (
            <div className="flex gap-3 px-4 py-4 border-b">
              ...
              <div className="flex-1">
                ...
                <div className="flex justify-end mt-2">
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={handleComment} disabled={!newComment.trim()}
                    className="px-4 py-1.5 rounded-full text-sm font-semibold text-white disabled:opacity-40" style={{ background: '#F97316' }}
                  >Gửi</motion.button>
                </div>
              </div>
            </div>
        ...
        <div>
          {comments.map((comment) => (
            <motion.div key={comment.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="px-4 py-3 border-b">
              <div className="flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-4 mt-2">
                    <button className="text-xs flex items-center gap-1" style={{ color: 'var(--text-secondary)' }}><Heart className="w-3.5 h-3.5" /> {comment.likesCount}</button>
                    <button onClick={() => {
                      if (!isAuthenticated) router.push('/login');
